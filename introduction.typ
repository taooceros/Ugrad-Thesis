...
In the current landscape of computational technology, the focus of
enhancing Central Processing Unit (CPU) performance has transitioned from increasing clock speeds to multiplying core counts. This evolution has given rise to multi-core architectures, which have become ubiquitous across
...
bottlenecks in a concurrent execution environment @ccsynch_ref. Theoretically, the synchronization duration should be invariant with
...
mechanism or are limited to fairness at the point of acquisition @scheduler_coop_locks_ref. They point out that one critical lock property, _lock usage_, is missing from the previous literature. Consider the scenario where two threads' critical sections size are different: e.g. the first thread take three times longer to finish its critical section compared to the second thread.
...
background tasks, all synchronized by a lock. In the absence of the principle of usage fairness, the interactive threads may suffer from inordinate
...
The non-preemptive nature of locks makes this problem hard. It is not allowed to switch lock owners when one thread hasn't finished its own critical section. Previous work offers locks that adopt usage-fairness, such as _SCLs_ @scheduler_coop_locks_ref. However, their solutions involve several limitations.
...
Recent research proposes _CFL_ @cfl_ref, which is work-conserving and reordered similar to the linux scheduler _CFS_. However, their solution still doesn't address the foundamental performance problem causes by lock owner switching.

== Performance issue of fair locks

Fundamentally, a fair lock means that the lock owner will be switching very frequently (presumably every single acquire if under high contention). However, this raises serious performance problem that makes lock foundamentally slower than sequencial programs.
...
_Delegation-styled_ locks have emerged as an innovative solution aimed at
boosting synchronization efficiency by minimizing contention and the
associated overhead of data movement. Instead of having each thread compete for a lock to execute their critical section, threads package their critical
...
Most delegation styled locks adopt either eventual acquisition fairness (@flatcombining_ref@rcl_ref@ffwd_ref) or acquisition fairness (@ccsynch_ref@tclocks_ref). However, none of them adopt usage fairness. The goal of this thesis is to bring delegation styled locks to a stage where they can demonstrate unparalleled advantages. We start by transforming existing delegation styled locks into variants that adopt usage fairness by implementing the banning strategy utilized in _SCLs_ --- _FC-Ban_ and _CCSynch-Ban_. Then we modify the inner structure with a concurrent re-ordering mechanism to produce a usage fair delegation styled lock --- _FC-Skiplist_. At the very last, we show how to adopt any kind of serialized scheduling policy with delegation semantics with _FC-Channel_.
